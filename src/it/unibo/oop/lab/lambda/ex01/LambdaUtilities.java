 ...
 * the first one is provided as exmaple.
 * 
...
public final class LambdaUtilities {
	...
	public static <T> List<Optional<T>> optFilter(final List<T> list, final Predicate<T> pre) {
		List<Optional<T>> listToReturn = new ArrayList<>();

		list.forEach(t -> {
			if (pre.test(t)) {
				listToReturn.add(Optional.ofNullable(t));
			}
			else {
				listToReturn.add(Optional.empty());
			}
		});
		/*
	...
	public static <R, T> Map<R, Set<T>> group(final List<T> list, final Function<T, R> op) {
		Map<R, Set<T>> mapToReturn = new HashMap<>();
		list.forEach(t -> {
			mapToReturn.merge(op.apply(t), new HashSet<>(Arrays.asList(t)), (t1, t2) -> {
                t1.addAll(t2);
                return t1;
            });
		});
	...
	public static <K, V> Map<K, V> fill(final Map<K, Optional<V>> map, final Supplier<V> def) {
		...
		map.forEach((k,v) ->  mapToReturn.put(k, v.orElseGet(def)));
